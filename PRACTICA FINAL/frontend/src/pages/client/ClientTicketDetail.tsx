...
import { getTicketById, getComments, addComment } from '../../services/ticketService';
...
const CommentBubble = ({ comment, isOwn }: { comment: Comment; isOwn: boolean }) => (
...
const ClientTicketDetail = () => {
  ...
  const handleSendComment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setComments((prev: Comment[]) => [...prev, created as unknown as Comment]);
      setNewComment('');
    } catch (err: any) {
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-1">
            ...
            <span className="text-xs text-slate-400">#{ticket.id.slice(0, 8).toUpperCase()}</span>
          </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="px-5 divide-y divide-slate-100">
          ...
          <DetailRow label="Prioridad">
            <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs ${priorityStyle.badge}`}>
          ...
          <DetailRow label="Asignado a">
            {ticket.assigned_to
              ? <span className="font-medium text-slate-700">{ticket.assigned_to}</span>
        ...
        <div className="px-5 py-4 space-y-4 min-h-[120px] max-h-[400px] overflow-y-auto">
          {loadingComments ? (
            ...
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              {isTicketOpen && (
                <p className="text-xs text-slate-400 mt-0.5">Puedes escribir uno abajo.</p>
              )}
            </div>
          ) : (
            comments.map((c) => (
        ...
        {isTicketOpen ? (
          ...
          <div className="border-t border-slate-100 px-5 py-3 bg-slate-50">
            <p className="text-xs text-slate-400 text-center">
              Este ticket está {STATUS_LABEL[ticket.status].toLowerCase()} y no admite nuevos comentarios.
            </p>
          </div>
        )}
      </div>
